...
while len(guessed_nums) < allowed_guesses:
    ...
    if not player_num > 0 and not player_num < 11:
        print("That number isn't betwen 1 and 10!")
        break
    ...
    if player_num == rand_num:
        ...
        if rand_num > player_num:
            print("Nope! My number is higher than {}. #{}".format(
                player_num, len(guessed_nums)
            ))
        else:
            print("Nope! My number is lower than {}. #{}".format(
                player_num, len(guessed_nums)
            ))
        continue
